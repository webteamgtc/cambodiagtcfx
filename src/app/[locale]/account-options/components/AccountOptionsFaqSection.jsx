...
import { FAQ_ITEMS } from "../accountOptionsData";
...
function FaqItem({ itemKey, isOpen, onToggle, t }) {
  return (
    <div
      className={clsx(
    ...
    >
      <button
        ...
        onClick={onToggle}
        className={clsx(
          "flex w-full items-center justify-between gap-4 px-4 py-4 text-left md:px-5",
        )}
        aria-expanded={isOpen}
      >
        <span className="Text font-medium text-[#000]">
...
export default function AccountOptionsFaqSection() {
  const t = usePathTranslation("accountOptionsPage");
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <section className="relative bg-[#F8F9FC] py-10 md:py-14">
      <div className="container min-w-0 max-w-full">
        <div className="mx-auto max-w-3xl text-center">
          <SectionEyebrow variant="compact" className="!text-[#293B93] capitalize">
            {t("faqSection.eyebrow", "FAQ")}
          </SectionEyebrow>

          <h2 className="HeadingH1 mt-4 font-semibold leading-[1.25] text-[#000]">
            {t("faqSection.title", "Frequently Asked Questions")}
          </h2>

          <p className="Text mx-auto mt-5 max-w-2xl font-normal leading-[1.7] text-[#000032]/60">
            {t(
              "faqSection.subtitle",
              "Everything you need to know about opening and managing your GTCFX trading account."
            )}
          </p>
