...
export default function CourseHeader({
  ...
  subtitle,
  progressTotal = 5,
  backHref = "/knowledge-to-learn",
  translationScope,
}) {
  const t = usePathTranslation(translationScope || "forTradingBeginnersPage.courseHeader");

  const displayTitle = translationScope ? t("title", title) : title;
  const displaySubtitle = translationScope ? t("subtitle", subtitle) : subtitle;
    ...
    <section className="bg-white py-10 md:py-16 lg:py-24">
      <div className="container">
        <div className="flex items-start gap-3">
          ...
          />
          <div>
            <h1 className="HeadingH2 font-bold text-black">{displayTitle}</h1>
            <p className="Text mt-3 text-[#666]">{displaySubtitle}</p>
          </div>
        </div>
        ...
        {/* <div className="mt-8 flex flex-col gap-4 md:mt-10 md:flex-row md:items-center md:gap-6">
          <div className="shrink-0">
            ...
            <p className="TextSmall text-[#666]">{`0 of ${progressTotal} finished`}</p>
          </div>

          <div className="flex-1">
            <div className="h-2 w-full rounded-full bg-[#E5EAF4]">
              <div className="h-2 rounded-full bg-[#293B93]" style={{ width: "0%" }} />
            </div>
          </div>
