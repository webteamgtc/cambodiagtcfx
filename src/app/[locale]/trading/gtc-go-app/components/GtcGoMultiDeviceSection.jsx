...
const DESKTOP_PLATFORM_KEYS = new Set(["windows", "macos", "linux", "webtrader"]);
...
export default function GtcGoMultiDeviceSection({ data, page }) {
  ...
  const tHome = usePathTranslation("home.invest");
  const locale = useLocale();
  const [activePlatform, setActivePlatform] = useState(multiDevice.defaultPlatform || "windows");

  return (
    <section className={`py-12 md:py-12 ${page === "symbolDetail" ? "bg-transparent" : "bg-white"}`}>
        ...
        <div className="mx-auto max-w-6xl text-center">
          {page === "symbolDetail" ? (
            <FadeInSection className="mx-auto max-w-xl text-center">
              ...
              <p className="Text mx-auto mt-4 max-w-3xl font-normal leading-[1.8] text-[#000032]/60 sm:mt-5">
                {tHome("sectionSubtitle")}
              </p>
            </FadeInSection>
          ) : (
            <FadeInSection>
          ...
          <FadeInSection delay={0.15}>
            ...
            <div className="mt-5 flex gap-2.5 sm:mt-6 lg:hidden">
              <AppStoreButton
                type="apple"
                variant="dark"
                className="min-w-0 flex-1 justify-center rounded-[14px] px-3 py-3.5"
              />
              <AppStoreButton
                type="google"
                variant="gold"
            ...
            <div className="mt-5 hidden gap-3 lg:flex lg:flex-wrap lg:items-center lg:justify-center">
              {multiDevice.platforms
                .map((platform, index) => ({ platform, index }))
                .filter(({ platform }) => DESKTOP_PLATFORM_KEYS.has(platform.key))
                .map(({ platform, index }) => {
