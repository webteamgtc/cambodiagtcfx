...
function PromoCard({ title, description, headingLevel = 3 }) {
  const HeadingTag = headingLevel === 3 ? "h3" : "p";
  return (
    <article className="rounded-[20px] p-5">
      <HeadingTag className="HeadingH5 text-left font-semibold leading-[1.35] text-[#000]">
        {title}
      </HeadingTag>
...
function ShowcaseColumn({ items, direction, duration, offset = 0, promo, t }) {
  const renderItem = (item, keySuffix = "", { headingLevel = 3 } = {}) => {
    if (item.type === "promo") {
      return (
        <PromoCard
          key={`promo-${keySuffix}`}
    ...
    >
      ...
      >
        ...
        <div className="flex w-full flex-col gap-4 sm:gap-5" aria-hidden>
          {items.map((item, index) => renderItem(item, `dup-${index}`, { headingLevel: 0 }))}
        </div>
      </div>
    </div>
  );
