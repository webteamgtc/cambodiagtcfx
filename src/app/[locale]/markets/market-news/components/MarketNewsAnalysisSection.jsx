  ...
  return (
    ...
    >
      ...
      <div className="absolute left-4 top-4 z-10 md:left-5 md:top-5">
        <p className="text-lg font-bold leading-none text-white md:text-xl">{t("brandLine1", "GTC")}</p>
        <p className="mt-1 text-[9px] font-medium uppercase tracking-[0.18em] text-white/70">{t("brandLine2", "Global Trade Capital")}</p>
      </div>
      ...
      <div className="pointer-events-none absolute bottom-0 right-0 top-0 w-[42%] md:w-[38%]">
        <Image
          src={presenterImage}
          alt={featured?.headlineLine1 || "Market analysis video"}
          fill
...
function FeaturedVideoPlayer({ item }) {
  return (
    <article className="relative overflow-hidden">
      <Image src={"/new-design/Markets/vedio-crop.webp"} className="w-full" alt={item.headlineLine1} width={1000} height={1000} loading="lazy" />
...
function VideoThumbnailCard({ item }) {
  return (
    <article className="overflow-hidden">
      <Image src={"/new-design/Markets/vedio-one.svg"} className="w-full min-h-[270px]" alt={item.headlineLine1} width={1000} height={1000} />
...
export default function MarketNewsAnalysisSection() {
  return (
      ...
      <div className="container min-w-0 max-w-full">
        <div className="mx-auto max-w-6xl">
          ...
          <FadeInSection delay={0.1}>
            <FeaturedVideoPlayer item={ANALYSIS_VIDEO.featured} />
          </FadeInSection>
          ...
          <div className="hidden md:grid mt-4 gap-4 md:grid-cols-3">
            {ANALYSIS_VIDEO.thumbnails.map((item, index) => (
              <VideoThumbnailCard key={`${item.headlineLine1}-${index}`} item={item} />
            ))}
          </div>
          </FadeInSection>
        </div>
